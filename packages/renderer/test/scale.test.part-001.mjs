...
export const { Renderer, loadGltfFromFile } = pkg

export const SIZE = 96
export const BACKGROUND = [5, 5, 5]
export const NODE_PERFORMANCE_NODE_COUNT = 10000
export const NODE_PERFORMANCE_IMAGE_COUNT = 100
export const LARGE_TEXTURE_SIZE = 512
export const NESTED_GRAPH_COLUMNS = 16
export const NESTED_GRAPH_ROWS = 16
export const NESTED_GRAPH_DEPTH = 8
...
export function makeTexture(index) {
  const size = 4
...
export function makeEncodedTexture(index) {
...
export function makeLargeTexture() {
...
export function makePngDataUri(index) {
  const raw = makeTexture(index)
  const image = raw.image
  const data = Buffer.from(image.data.buffer, image.data.byteOffset, image.data.byteLength)
...
export const CRC32_TABLE = Array.from({ length: 256 }, (_, index) => {
  let value = index
...
export function makeSharedTriangleBuffer() {
  const arrays = [
...
export function makeNodePerformanceGltfSource() {
...
export function addSupportedLightBudget(scene, count = 8) {

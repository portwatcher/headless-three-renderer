...
- **`gltf.test.mjs`** — local glTF fixtures load geometry/materials, vertex colors, morph targets, skinning attributes, encoded data-URI textures, external image files, and external buffers through the exported local glTF helper, including the loader plugin/configuration hook, and render through the documented Node loader setup.
...
- **`golden.test.mjs`** — optional browser-reference comparison for the generated corpus. It is skipped unless `HEADLESS_THREE_BROWSER_REFERENCE_DIR` points at PNGs generated by `test/browser-reference/index.html`; CI runs the skipped harness so the entry point stays covered until references are committed.

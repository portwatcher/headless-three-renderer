...
- **`gltf.test.mjs`** — local glTF fixtures load geometry/materials, vertex colors, morph targets, skinning attributes, encoded data-URI textures, GLB bufferView images, synthetic VRM/VRMA extension fixtures, external image files, external buffers, and committed Khronos glTF Sample Assets through the exported local glTF helper, including external and embedded compressed-image failure guidance, the loader plugin/configuration hook, and render through the documented Node loader setup.
...
The generated corpus, minimal local glTF fixtures, and committed Khronos glTF Sample Assets are broader no-crash/visibility sweeps and run in CI; committed browser-generated golden images and real external VRM/VRMA assets remain future work.

...
- **`gltf.test.mjs`** — local glTF fixtures load geometry/materials, vertex colors, morph targets, skinning attributes, encoded data-URI textures, GLB bufferView images, external image files, and external buffers through the exported local glTF helper, including the loader plugin/configuration hook, and render through the documented Node loader setup.

...
- **`scenes.test.mjs`** — scene-level invariants: `rgba` format produces `width * height * 4` bytes, meshes cover reasonable portions of the frame, different materials produce different pixel statistics, LOD distance/zoom/hysteresis selection behaves consistently, EffectComposer helper flows including CopyShader passes and OutputPass render through target readback, examples helpers cover generated geometry, transform, morph-animation, and utility paths, PBR scenes show lighting gradients, line/point topologies render without error, empty scene renders background color.

...
- **`gltf.test.mjs`** — local glTF fixtures load geometry/materials, vertex colors, morph targets, skinning attributes, encoded data-URI textures, GLB bufferView images, synthetic and real external VRM/VRMA extension fixtures, external image files, external buffers, and committed Khronos glTF Sample Assets through the exported local glTF helper, including external and embedded compressed-image failure guidance, the loader plugin/configuration hook, and render through the documented Node loader setup.

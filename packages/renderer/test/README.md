...
- **`corpus.test.mjs`** — generated representative scenes render without crashes and produce visible non-background pixels for transparent layers, custom blend equations/factors, 4x-MSAA alpha-to-coverage, stencil-masked render state, camera/object/light layer filtering, ArrayCamera/CubeCamera outputs including update capture, focused depth/distance material output, Standard displacement output, 2D/equirectangular/cube and packed CubeUV background textures plus renderer-only raw and CubeUV cube background textures, signed raw DataTextures, browser-reference alpha/AO/emissive/Phong specular explicit texture matrices and light-map texture coverage, toon texture-slot routing, toon alpha-map cutouts, LightProbe lit-material models, renderer-only LightProbe/environment material models, material env maps including renderer-only CubeUV cube and packed CubeUV material env maps, mask/object-id/normal/depth render modes, renderer-only MRT render-mode attachments, ShadowMaterial opacity scaling, output color-space conversion, and fog opt-out, skinned/morphed geometry, avatar-style toon/skinned combinations, physical IBL + shadows, renderer-only physical clearcoat, sheen, specular, anisotropy, iridescence, and transmission/thickness maps, browser-reference mixed directional/spot/point shadow lights and shadow-map gating plus renderer-only custom/source shadow displacement maps, renderer-only sprite/point billboard point-light shadows, browser-reference LineBasicMaterial selected UV-channel routing, renderer-only custom-distance and LineLoop dashed lines, instanced points/lines plus renderer-only Line/LineLoop no-bridge checks, browser-reference BatchedMesh instance/range/repack/indexed-group/group/culling-opt-out/sort fixtures plus renderer-only multi-source group offsets, LOD/groups, and pathological geometry.
- **`golden.test.mjs`** — browser-reference comparison for the generated corpus. It uses committed platform references when `browser-reference/references/<platform>-<arch>/` exists, accepts explicit `HEADLESS_THREE_BROWSER_REFERENCE_DIR` inputs, requires the committed Linux x64 references in CI, runs automatically on platforms with committed default references, and otherwise skips platforms without committed references.
...
- **`scenes.test.mjs`** — scene-level invariants: `rgba` format produces `width * height * 4` bytes, meshes cover reasonable portions of the frame, different materials produce different pixel statistics, LOD distance/zoom/hysteresis selection behaves consistently, EffectComposer helper flows including CopyShader passes and OutputPass render through target readback, examples helpers cover generated/debris geometry, transform, morph-animation, collision math, and utility paths, PBR scenes show lighting gradients, line/point topologies render without error, empty scene renders background color.
...
The generated corpus, committed Linux x64 and macOS arm64 browser references, minimal local glTF fixtures, committed humanoid/full-avatar VRM/VRMA fixtures, and committed Khronos glTF Sample Assets are broader no-crash/visibility sweeps and run in CI; browser references for the remaining supported platforms remain future work.

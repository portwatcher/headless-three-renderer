...
- **`corpus.test.mjs`** — generated representative scenes render without crashes and produce visible non-background pixels for transparent layers, custom blend equations/factors, 4x-MSAA alpha-to-coverage, stencil-masked render state, ArrayCamera/CubeCamera outputs including update capture, focused depth/distance material output, 2D/equirectangular/cube and packed CubeUV background textures plus renderer-only raw and CubeUV cube background textures, signed raw DataTextures, renderer-only alpha/AO/emissive explicit texture matrices, renderer-only light-map texture coverage, toon alpha-map cutouts, LightProbe lit-material models, renderer-only LightProbe/environment material models, material env maps including renderer-only CubeUV cube and packed CubeUV material env maps, mask/object-id/normal/depth render modes, ShadowMaterial fog opt-out, skinned/morphed geometry, avatar-style toon/skinned combinations, physical IBL + shadows, renderer-only mixed directional/spot/point shadow lights, renderer-only sprite/point billboard point-light shadows, renderer-only LineBasicMaterial selected UV-channel routing, renderer-only custom-distance dashed lines, instanced points/lines, BatchedMesh instance/range/group/culling/sort fixtures, LOD/groups, and pathological geometry.

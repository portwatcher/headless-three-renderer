...
test('custom shadow material alphaMaps sample selected UV channels', () => {
  function alphaMapForChannel(channel) {
    const alphaMap = rgbaTexture([
      255, 0, 255, 255,
      255, 255, 255, 255,
  ...
  function renderDirectionalCustomDepthChannel(channel) {
    const scene = new THREE.Scene()
    scene.background = new THREE.Color(1, 1, 1)
    addReceiver(scene)
  ...
  function renderPointCustomDistanceChannel(channel) {
    const scene = new THREE.Scene()
    scene.background = new THREE.Color(1, 1, 1)
    addReceiver(scene)
...
test('custom and source shadow base maps sample selected UV channels', () => {
  ...
  function sourceMaterialWithBaseMap(channel) {
    return new THREE.MeshBasicMaterial({
      color: 0xffffff,
      map: baseMapForChannel(channel),
      alphaTest: 0.5,
  ...
  function renderDirectionalCustomDepthBaseChannel(channel, inheritFromSource) {
    const scene = new THREE.Scene()
    scene.background = new THREE.Color(1, 1, 1)
    addReceiver(scene)
  ...
  function renderPointCustomDistanceBaseChannel(channel, inheritFromSource) {
    const scene = new THREE.Scene()
    scene.background = new THREE.Color(1, 1, 1)
    addReceiver(scene)
...
test('custom and source shadow alphaMaps apply texture UV transforms', () => {
  function alphaMapWithOffset(offsetX) {
    const alphaMap = rgbaTexture([
      255, 0, 255, 255,
      255, 255, 255, 255,
    ], 2, 1)
  ...
  function sourceMaterialWithAlphaMap(offsetX) {
    return new THREE.MeshBasicMaterial({
      color: 0xffffff,
      alphaMap: alphaMapWithOffset(offsetX),
      alphaTest: 0.5,
      colorWrite: false,
      depthWrite: false,
    })
  ...
  function renderDirectionalCustomDepthAlphaTransform(offsetX, inheritFromSource) {
    const scene = new THREE.Scene()
    scene.background = new THREE.Color(1, 1, 1)
    addReceiver(scene)
    ...
    const caster = new THREE.Mesh(
      casterGeometry(3),
      inheritFromSource
        ? sourceMaterialWithAlphaMap(offsetX)
        : new THREE.MeshBasicMaterial({ color: 0xffffff }),
    )
    caster.position.y = 1.5
    caster.castShadow = true
  ...
  function renderPointCustomDistanceAlphaTransform(offsetX, inheritFromSource) {
    const scene = new THREE.Scene()
    scene.background = new THREE.Color(1, 1, 1)
    addReceiver(scene)
  ...
  for (const inheritFromSource of [false, true]) {
    const label = inheritFromSource ? 'source material' : 'custom shadow material'
    const transparentDepth = renderDirectionalCustomDepthAlphaTransform(0, inheritFromSource)
    const offsetDepth = renderDirectionalCustomDepthAlphaTransform(0.5, inheritFromSource)
    const transparentDepthLum = transparentDepth.r + transparentDepth.g + transparentDepth.b
    const offsetDepthLum = offsetDepth.r + offsetDepth.g + offsetDepth.b
    assert.ok(
      offsetDepthLum < transparentDepthLum - 30,
      `${label} alphaMap offset should shift customDepthMaterial caster UVs into the opaque texel (${offsetDepthLum} vs ${transparentDepthLum})`,
    )

    const transparentDistance = renderPointCustomDistanceAlphaTransform(0, inheritFromSource)
    const offsetDistance = renderPointCustomDistanceAlphaTransform(0.5, inheritFromSource)
    const transparentDistanceLum = transparentDistance.r + transparentDistance.g + transparentDistance.b
    const offsetDistanceLum = offsetDistance.r + offsetDistance.g + offsetDistance.b
    assert.ok(
      offsetDistanceLum < transparentDistanceLum - 20,
      `${label} alphaMap offset should shift customDistanceMaterial caster UVs into the opaque texel (${offsetDistanceLum} vs ${transparentDistanceLum})`,
    )
  }
})

test('custom and source shadow base maps apply texture UV transforms', () => {
  function baseMapWithOffset(offsetX) {
    const map = rgbaTexture([
      255, 255, 255, 0,
      255, 255, 255, 255,
    ], 2, 1)
  ...
  function sourceMaterialWithBaseMap(offsetX) {
    return new THREE.MeshBasicMaterial({
      color: 0xffffff,
      map: baseMapWithOffset(offsetX),
      alphaTest: 0.5,
      colorWrite: false,
      depthWrite: false,
    })
  ...
  function renderDirectionalCustomDepthBaseTransform(offsetX, inheritFromSource) {
    const scene = new THREE.Scene()
    scene.background = new THREE.Color(1, 1, 1)
    addReceiver(scene)
    ...
    const caster = new THREE.Mesh(
      casterGeometry(3),
      inheritFromSource
        ? sourceMaterialWithBaseMap(offsetX)
        : new THREE.MeshBasicMaterial({ color: 0xffffff }),
    )
    caster.position.y = 1.5
    caster.castShadow = true
  ...
  function renderPointCustomDistanceBaseTransform(offsetX, inheritFromSource) {
    const scene = new THREE.Scene()
    scene.background = new THREE.Color(1, 1, 1)
    addReceiver(scene)
  ...
  for (const inheritFromSource of [false, true]) {
    const label = inheritFromSource ? 'source material' : 'custom shadow material'
    const transparentDepth = renderDirectionalCustomDepthBaseTransform(0, inheritFromSource)
    const offsetDepth = renderDirectionalCustomDepthBaseTransform(0.5, inheritFromSource)
    const transparentDepthLum = transparentDepth.r + transparentDepth.g + transparentDepth.b
    const offsetDepthLum = offsetDepth.r + offsetDepth.g + offsetDepth.b
    assert.ok(
      offsetDepthLum < transparentDepthLum - 30,
      `${label} map offset should shift customDepthMaterial caster UVs into the opaque texel (${offsetDepthLum} vs ${transparentDepthLum})`,
    )

    const transparentDistance = renderPointCustomDistanceBaseTransform(0, inheritFromSource)
    const offsetDistance = renderPointCustomDistanceBaseTransform(0.5, inheritFromSource)
    const transparentDistanceLum = transparentDistance.r + transparentDistance.g + transparentDistance.b
    const offsetDistanceLum = offsetDistance.r + offsetDistance.g + offsetDistance.b
    assert.ok(
      offsetDistanceLum < transparentDistanceLum - 20,
      `${label} map offset should shift customDistanceMaterial caster UVs into the opaque texel (${offsetDistanceLum} vs ${transparentDistanceLum})`,
    )
  }
})
...
test('MeshPhysicalMaterial iridescenceMap samples selected uv1-uv3 texture channels', () => {
  ...
  const primary = renderIridescenceMap(0)
  for (const channel of [1, 2, 3]) {
    const diff = meanAbsDiff(primary, renderIridescenceMap(channel))
    assert.ok(diff > 10, `iridescenceMap channel=${channel} should sample uv${channel}'s texel that enables iridescence (${diff.toFixed(2)})`)
  }
})
...
test('MeshPhysicalMaterial iridescenceThicknessMap samples selected uv1-uv3 texture channels', () => {
  ...
  const primary = renderThicknessMap(0)
  for (const channel of [1, 2, 3]) {
    const diff = meanAbsDiff(primary, renderThicknessMap(channel))
    assert.ok(diff > 5, `iridescenceThicknessMap channel=${channel} should sample uv${channel}'s texel for the upper film thickness range (${diff.toFixed(2)})`)
  }
})

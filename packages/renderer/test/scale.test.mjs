...
test('mesh render budget handles 1,024 separate mesh objects', () => {
  const scene = new THREE.Scene()
  scene.background = new THREE.Color(0.02, 0.02, 0.02)

  const columns = 32
  const rows = 32
  const geometry = new THREE.PlaneGeometry(0.048, 0.048)
  const materials = [
  ...
  for (let row = 0; row < rows; row += 1) {
    for (let col = 0; col < columns; col += 1) {
      ...
      mesh.position.set((col - (columns - 1) / 2) * 0.064, (row - (rows - 1) / 2) * 0.064, 0)

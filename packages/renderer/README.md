...
- `Renderer.backend` exposes inert CommonRenderer backend probes for backend type flags, coordinate-system/readback metadata, feature/anisotropy checks, drawing-buffer size, DOM element access, and scratch backend data; direct backend context, storage-buffer, timestamp, and GPU-sync operations fail clearly

...
- `options.outputColorSpace`: `THREE.SRGBColorSpace` (`'srgb'`, default) or `THREE.LinearSRGBColorSpace` (`'srgb-linear'`, `'linear-srgb'`, `'linearsrgb'`, or `'linear'`) for material and texture background output conversion; reusable `Renderer.outputColorSpace` and `_outputColorSpace` provide the same value as a renderer-state fallback, and unsupported values fail clearly.
...
- `Renderer.transmissionResolutionScale` is stored as positive finite WebGLRenderer compatibility state; exact downscaled transmission rendering remains planned
...
- `Renderer.state` exposes a minimal inert `buffers.depth.getReversed()` probe plus no-op `reset()`/`unbindTexture()` hooks for WebGLRenderer-adjacent helpers
...
- base, sprite, point, line/dashed-line, matcap, emissive, light, Phong specular, sheen color, and physical specular color maps honor `THREE.SRGBColorSpace`/`THREE.LinearSRGBColorSpace`, including the documented linear string aliases; unsupported texture color-space/encoding values fail clearly

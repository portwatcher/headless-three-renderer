...
- mipmap min filters generate native mip chains from supported material/background texture source levels; raw `DataTexture`-style explicit mipmap arrays upload for unpacked 2D material/background texture slots, while malformed texture source containers plus packed physical-extension maps and environment/reflection-probe explicit mip arrays fail clearly

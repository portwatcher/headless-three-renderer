...
`THREE.Line`, `THREE.LineSegments`, `THREE.LineLoop`, and `THREE.Points` are supported. Lines and points render as unlit (basic) primitives and ignore lighting / normals. Opacity, scene fog, and `material.fog = false` are honored. `LineBasicMaterial.map` samples line UVs, including texture UV transforms, `texture.channel` UV selection, texture RGB with sRGB color-space decode, and alpha-tested texture alpha; dashed lines honor custom `lineDistance` attributes, and dashed line maps preserve texture UV transforms and the selected UV channel while reconstructing dash segments. `PointsMaterial` maps and alpha maps use point-sprite UVs, honor texture UV transforms, decode sRGB color maps, shrink with perspective distance by default, keep orthographic point size independent of camera depth, and cast directional/spot shadows from the expanded billboard quads. `THREE.Points.receiveShadow` and point-light point-billboard shadows fail clearly until those paths land.

...
- `Object3D.customDepthMaterial` and `customDistanceMaterial` for mesh shadow caster alpha-tested inputs, including selected base-map and alpha-map UV channels, displacement, and wireframe material inputs, plus source-material base/alpha maps with selected base-map and alpha-map UV channels, opacity, clipping, displacement, `shadowSide`, and wireframe state on custom shadow casters, and alpha-tested sprite/point billboard shadow cutouts; malformed custom shadow material containers fail clearly

...
- `camera`: a `THREE.Camera`, including perspective and orthographic cameras. `THREE.ArrayCamera` composes sub-camera viewports for PNG, raw RGBA, and target output. `THREE.CubeCamera` renders six RGBA faces into `WebGLCubeRenderTarget.texture.image`/`source.data`; cube mip/depth targets and exact WebGL face semantics remain limited.

...
- `Object3D.customDepthMaterial` and `customDistanceMaterial` for mesh shadow caster alpha-tested inputs, including selected base-map and alpha-map UV channels, base-map and alpha-map texture transforms, displacement, and wireframe material inputs, plus source-material base/alpha maps with selected base-map and alpha-map UV channels, base-map and alpha-map texture transforms, opacity, clipping, displacement, `shadowSide`, and wireframe state on custom shadow casters, alpha-tested sprite/point billboard shadow cutouts, and point-billboard custom-distance base-map/alpha-map cutouts with selected geometry UV channels; malformed custom shadow material containers fail clearly

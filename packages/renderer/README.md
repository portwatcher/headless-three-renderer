...
- render-option viewport/scissor rectangles, render-target dimensions and viewport/scissor fields, reusable `Renderer.setSize()`/`getSize()` plus `setViewport()`/`setScissor()`/`setScissorTest()` state in output pixel coordinates, no-op `clear()`/`clearColor()`/`clearDepth()`/`clearStencil()`/`dispose()` hooks, and no-op `autoClear`/`autoClearColor`/`autoClearDepth`/`autoClearStencil` flags for pass-owned buffers and object-lifetime native cleanup, with invalid sizes, clear booleans, auto-clear flags, and malformed target scissor flags failing clearly

...
- `Renderer.backend` exposes inert CommonRenderer backend probes for backend type flags, coordinate-system/readback metadata, lifecycle begin/finish/update hooks, scissor/occlusion/clear-color probes, feature/anisotropy checks, drawing-buffer size, DOM element access, cleanup/cache-key probes, and scratch backend data; direct backend context, draw/program/binding/pipeline/node-builder/texture/attribute/copy, storage-buffer, timestamp, and GPU-sync operations fail clearly
- `Renderer.nodes`, `Renderer.library`, and `Renderer.lighting` expose inert CommonRenderer shader-node registry and lighting-node compatibility state; actual Three.js shader graph translation still requires the documented custom WGSL path

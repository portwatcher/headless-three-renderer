...
- render-option viewport/scissor rectangles, render-target dimensions and viewport/scissor fields, reusable `Renderer.setSize()`/`getSize()` plus `setDrawingBufferSize()`/`getDrawingBufferSize()` and `setPixelRatio()`/`getPixelRatio()` compatibility state, inert `Renderer.domElement` output-size/style mirror, `Renderer.outputColorSpace` output-conversion state, `Renderer.toneMapping`/`toneMappingExposure` material tone-mapping state, `Renderer.clippingPlanes` global clipping fallback state, `Renderer.localClippingEnabled` material-local clipping fallback state, `Renderer.info` inert compatibility counters, inert `Renderer.debug` shader-diagnostic state, conservative `Renderer.capabilities` draw-buffer/texture probes, inert `Renderer.extensions`, scratch `Renderer.properties`/`renderLists` helpers, basic inert `Renderer.xr` enabled/presentation/session probes, validated inert WebGLRenderer-style constructor parameters for common context attributes with cloned `getContextAttributes()` readback and clear `getContext()` unsupported failures, `setViewport()`/`setScissor()`/`setScissorTest()` state in output pixel coordinates, `Renderer.shadowMap.enabled` shadow gating plus `shadowMap.autoUpdate`/`needsUpdate`/`type` compatibility state, clear color/depth/stencil value state, no-op `compile()`/`compileAsync()`/`initRenderTarget()`/`initTexture()`/`clear()`/`clearColor()`/`clearDepth()`/`clearStencil()`/`resetState()`/`dispose()`/`forceContextLoss()`/`forceContextRestore()`/`setAnimationLoop()` hooks, and no-op `autoClear`/`autoClearColor`/`autoClearDepth`/`autoClearStencil` flags for pass-owned buffers and object-lifetime native cleanup; `getContext()`, `domElement.getContext()`, `setRenderTargetTextures()`, `setRenderTargetFramebuffer()`, `copyFramebufferToTexture()`, and `copyTextureToTexture()` fail clearly because this package has no browser WebGL context, external WebGL texture/framebuffer binding, persistent browser framebuffer, or GPU texture-copy API. Invalid sizes, pixel ratios, constructor parameters, output color spaces, tone-mapping constants/exposure values, clipping plane values/booleans, info booleans, debug values, capability/extension/property/render-list probes, XR booleans, shadow-map booleans/type constants, clear values/booleans, resource-init targets/textures, external WebGL texture/framebuffer handles, copy targets and textures, animation-loop callbacks, auto-clear flags, and malformed target scissor flags fail clearly

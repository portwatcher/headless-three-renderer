...
- `material.map` (base color texture) — PNG, JPEG, WebP, and raw one-channel, two-channel, RGB, or RGBA numeric DataTexture inputs, including byte, float, and half-float typed data, with `texture.channel` UV selection and sRGB color-space decode
...
- Raw one-channel, two-channel, RGB, or RGBA numeric pixels via `THREE.DataTexture` (or any image with `.data`, `.width`, `.height`), including `UnsignedByteType`, normalized float arrays, and `HalfFloatType` `Uint16Array` binary16 data

...
- `options.target`: a non-array target-like object populated for a single color output, including `target.texture`, `target.textures[0]`, or one-element `target.texture` arrays. Top-level `target.data` remains raw RGBA8; color textures can also request normalized `FloatType`, `UnsignedShortType`, `UnsignedIntType`, or `HalfFloatType` readback arrays.

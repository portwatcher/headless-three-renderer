...
@group(0) @binding(1) var y_plane: texture_storage_2d<r16unorm, write>;
@group(0) @binding(2) var uv_plane: texture_storage_2d<rg16unorm, write>;

fn p010_word(code: f32) -> f32 {
  return round(code) * 64.0 / 65535.0;
}

fn limited_y(rgb: vec3<f32>) -> f32 {
  return p010_word(64.0 + 876.0 * dot(rgb, vec3<f32>(0.2126, 0.7152, 0.0722)));
}
...
fn limited_uv(rgb: vec3<f32>) -> vec2<f32> {
  ...
  return vec2<f32>(p010_word(512.0 + 896.0 * cb), p010_word(512.0 + 896.0 * cr));
}

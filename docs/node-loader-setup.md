...
values, POSIX absolute paths and Windows drive-letter paths are preserved as
local files, and `file://` asset URLs are decoded with Node's `fileURLToPath()`.
URI-encoded local filenames such as
...
HTTP(S) asset or root URLs are rejected by the local helpers.
...
`itemStart`/`itemEnd`/`itemError` hooks and `resolveURL()` URL modifiers when a
manager is supplied. `setPath()` accepts relative, absolute, and `file://`
directory prefixes for later relative image loads before URL modifiers run,
without rewriting data URI, Blob URL, absolute, or fully-qualified asset URLs.
The renderer decodes those bytes natively, so no DOM `Image`, canvas, or WebGL
context is needed for external image files or PNG/JPEG/WebP data URI or Blob URL
image references.

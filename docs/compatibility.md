...
| Clipping planes | Partial | Render-option global `clippingPlanes` and material-local `clippingPlanes`/`clipIntersection` are evaluated in the main pass for meshes, sprites, points, and lines, up to eight world-space planes. `material.clipShadows` clips shadow-caster fragments in the shadow pass. `options.localClippingEnabled: false` disables material-local planes while preserving global planes. Scene-level `THREE.ClippingGroup` inputs fail clearly; `ClippingGroup` semantics, alpha-to-coverage smoothing, and exact golden parity remain planned. |
...
| Shadow behavior parity | Partial | Supports common camera bounds, rectangular directional/spot/cascaded map sizes, square point-light cube-face maps, bias, normal bias, `shadow.radius` PCF scaling, cast/receive flags, and alpha-tested casters. Non-square point-light shadow map sizes, non-default shadow blurSamples settings, and `customDepthMaterial`/`customDistanceMaterial` shadow overrides fail clearly; native multiple-shadow-map rendering, custom shadow material translation, and blurSamples support remain planned. |

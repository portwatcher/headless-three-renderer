...
| `renderToTarget()` / `options.target` | Supported | Populates target-like `{ width, height, data, texture.image.data }` fields with RGBA8 for a single color output, including one-element `target.texture` arrays and `target.textures[0]`. Target `depthTexture` objects receive RGBA8 normalized depth readback. Unsupported MRT and MSAA fields fail clearly. |
...
| Depth output / MRT | Partial | Target `depthTexture` objects receive RGBA8 normalized depth readback for single-output targets. Multiple color attachment requests fail clearly; native float depth texture readback and MRT remain planned. |

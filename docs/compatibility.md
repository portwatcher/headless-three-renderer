...
| `THREE.Line`, `LineSegments`, `LineLoop` | Partial | Rendered as unlit line lists. Common instanced-geometry offset/color attributes are CPU-expanded for non-dashed lines. `LineBasicMaterial.map` samples line UVs, including alpha-tested texture alpha. `LineDashedMaterial` dash/gap patterns are CPU-expanded for common non-instanced cases with reconstructed map UVs and interpolated vertex colors. Non-default `linewidth` and dashed instanced geometry fail clearly; thick-line support and broader line material options remain limited. |
| `THREE.Points` | Partial | `PointsMaterial` is CPU-expanded into camera-facing quads with size, perspective size attenuation, maps, alpha maps, vertex colors, layers, render ordering, main-pass clipping, and common instanced-geometry offset/color attributes. Hardware point-size caps, exact circular point-sprite edge cases, dashed line instancing parity, and shadows remain planned. |
...
| `InstancedBufferGeometry` | Partial | CPU-expanded for mesh, point, and non-dashed line geometry with instanced vertex colors and common instanced offset attributes (`instanceOffset`, `instancePosition`, `offset`, `translate`, `translation`). Arbitrary custom vertex attributes and dashed line instancing still require additional strategy. |

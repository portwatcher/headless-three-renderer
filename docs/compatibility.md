...
| Shadow behavior parity | Partial | Supports common camera bounds, rectangular directional/spot/cascaded map sizes, square point-light cube-face maps, bias, normal bias, cast/receive flags, and alpha-tested casters. Non-square point-light shadow map sizes, non-default shadow blur/radius settings, and `customDepthMaterial`/`customDistanceMaterial` shadow overrides fail clearly; native multiple-shadow-map rendering, custom shadow material translation, and blur/radius support remain planned. |

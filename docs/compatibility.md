...
| Clipping planes | Partial | Render-option global `clippingPlanes` and material-local `clippingPlanes`/`clipIntersection` are evaluated in the main pass for meshes, sprites, points, and lines, up to eight world-space planes. `material.clipShadows` and scene-level `THREE.ClippingGroup` inputs fail clearly; `WebGLRenderer.localClippingEnabled`, `ClippingGroup` semantics, shadow-pass clipping support, alpha-to-coverage smoothing, and exact golden parity remain planned. |

...
| Mask/object-id render modes | Partial | `options.renderMode: "mask"` writes white visible geometry on black; `"object-id"` encodes adapter object sort IDs plus one into RGB bytes with zero reserved for background. These passes bypass lighting, environment, fog, post-processing, and scene backgrounds while preserving depth, culling, clipping, base texture alpha, `alphaTest`, and `alphaHash`. `material.alphaMap` cutouts fail clearly until custom-pass alpha-map support lands. |

...
| `renderToTarget()` / `options.target` | Supported | Populates target-like `{ width, height, data, texture.image.data }` fields with RGBA8 for a single color output, including one-element `target.texture` arrays and `target.textures[0]`. Target `depthTexture` objects receive RGBA8 normalized depth readback, including scissor clipping and alpha-tested cutouts. Unsupported MRT and MSAA fields fail clearly. |
...
| Environment IBL | Supported | CPU precomputes diffuse irradiance, prefiltered specular, and BRDF LUT from equirectangular inputs and raw or encoded six-face cube reflection inputs. Standard/physical materials use diffuse/specular IBL, Phong materials use an approximate specular reflection path, Basic materials use legacy env-map combine modes for supported material-level reflection/refraction maps, and explicit `THREE.SRGBColorSpace`/`THREE.LinearSRGBColorSpace` is honored for scene-environment, reflection-probe, and supported material-level LDR inputs. Material-level `envMap` support is limited to one shared map and one shared rotation for Basic/standard/physical/Phong/Lambert material paths; non-Basic refraction and PMREM/CubeUV environment mappings fail clearly. |

...
| `THREE.Sprite` / billboards | Partial | CPU-expanded into camera-facing quads with `SpriteMaterial` color, opacity, scene fog and `material.fog` opt-out, map with texture UV transforms and sRGB color-space decode, alphaMap with UV transforms, center, scale, rotation, size attenuation, layers, render ordering, main-pass clipping, and directional/spot shadow casting. `receiveShadow` and point-light sprite shadows fail clearly; exact WebGL edge cases remain planned. |
...
| Shadow behavior parity | Partial | Supports common camera bounds, rectangular directional/spot/cascaded map sizes, square point-light cube-face maps, bias, normal bias, `shadow.radius` PCF scaling, PCF-irrelevant `shadow.blurSamples` inputs, cast/receive flags, texture alpha-tested casters, directional/spot sprite and point billboard shadow casters, and custom depth/distance shadow material alpha cutouts and displacement. Non-square point-light shadow map sizes plus point-light sprite/point-billboard shadow casters fail clearly; native multiple-shadow-map rendering, broader custom shadow material translation, and VSM-specific blur behavior remain planned. |

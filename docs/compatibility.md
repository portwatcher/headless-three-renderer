...
| `THREE.Line`, `LineSegments`, `LineLoop` | Partial | Rendered as unlit line lists. Common instanced-geometry offset/color attributes are CPU-expanded. `LineBasicMaterial.map` samples line UVs, including alpha-tested texture alpha. `LineDashedMaterial` dash/gap patterns are CPU-expanded for common non-instanced and instanced cases with reconstructed map UVs and interpolated vertex colors. Non-default `linewidth` fails clearly; thick-line support and broader line material options remain limited. |

...
| `renderToTarget()` / `options.target` | Supported | Populates target-like `{ width, height, data, texture.image.data }` fields with RGBA8 for a single color output, including one-element `target.texture` arrays and `target.textures[0]`. Target `depthTexture` objects receive RGBA8 normalized depth readback, including scissor clipping and alpha-tested cutouts. 4x MSAA resolves into target readback buffers; multiple color attachments and unsupported sample counts fail clearly. |
...
| Array/cube cameras | Partial | `THREE.ArrayCamera` sub-cameras compose into PNG, RGBA, and target output using each sub-camera viewport, including target depth readback. `THREE.CubeCamera` fails clearly; cube-camera targets and exact WebGL viewport semantics remain planned. |
...
| Depth output / MRT | Partial | Target `depthTexture` objects receive RGBA8 normalized depth readback for single-output targets, including scissor clipping and alpha-tested visible geometry. Multiple color attachment requests fail clearly; native float depth texture readback and MRT remain planned. |

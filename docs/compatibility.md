...
| `THREE.Points` | Partial | `PointsMaterial` is CPU-expanded into camera-facing quads with size, opacity, perspective size attenuation, depth-independent orthographic sizing, point-sprite map/alpha-map UVs, map/alpha-map UV transforms, sRGB color-space decode for maps, vertex colors, scene fog and `material.fog` opt-out, layers, render ordering, main-pass clipping, and common instanced-geometry offset/color attributes. Explicit point shadow flags fail clearly; hardware point-size caps, exact circular point-sprite edge cases, and shadow rendering remain planned. |

...
| Environment IBL | Supported | CPU precomputes diffuse irradiance, prefiltered specular, and BRDF LUT from equirectangular inputs and raw or encoded six-face cube reflection inputs. Standard/physical materials use diffuse/specular IBL, Phong materials use an approximate specular reflection path, and explicit `THREE.SRGBColorSpace`/`THREE.LinearSRGBColorSpace` is honored for scene-environment, reflection-probe, and supported material-level LDR inputs. Material-level reflection `envMap` support is limited to one shared map for standard/physical/Phong/Lambert material paths; refraction and PMREM/CubeUV environment mappings fail clearly. |

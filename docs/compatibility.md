...
| Mask/object-id render modes | Partial | `options.renderMode: "mask"` writes white visible geometry on black; `"object-id"` encodes adapter object sort IDs plus one into RGB bytes with zero reserved for background. These passes bypass lighting, environment, fog, post-processing, and scene backgrounds while preserving depth, culling, clipping, base texture alpha, `material.alphaMap`, `alphaTest`, and `alphaHash`. Reverse lookup metadata and MRT integration remain planned. |

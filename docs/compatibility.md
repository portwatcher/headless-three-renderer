...
| Array/cube cameras | Partial | `THREE.ArrayCamera` sub-cameras compose into PNG, RGBA, and target output using each sub-camera viewport, including target depth readback. `THREE.CubeCamera` renders six RGBA faces into `WebGLCubeRenderTarget.texture.image`/`source.data`; cube mip/depth targets and exact WebGL viewport/face semantics remain planned. |

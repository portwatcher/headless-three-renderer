...
| `MeshPhysicalMaterial` | Partial | Clearcoat, sheen, anisotropy, specular intensity/color, roughness-aware scene-color transmission, IOR, thickness, transmission volume attenuation color/distance, and related maps are supported. Iridescence and dispersion fail clearly; exact attenuation parity, transmission mip/bicubic behavior, and several edge cases remain planned. |
...
| Scale budget tests | Supported | CI renders many-mesh, many-texture, and supported 16-light budget scenes, and checks the clear failure path for larger visible non-ambient light sets. Covered budgets and platform notes are documented in [scale-budgets.md](./scale-budgets.md). |

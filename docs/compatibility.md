...
| `InstancedBufferGeometry` | Partial | CPU-expanded for mesh geometry with instanced vertex colors and common instanced offset attributes (`instanceOffset`, `instancePosition`, `offset`, `translate`, `translation`). Arbitrary custom vertex attributes still require a shader strategy. |
...
| `MeshPhysicalMaterial` | Partial | Clearcoat, sheen, anisotropy, specular intensity/color, transmission, IOR, thickness, attenuation, and related maps are supported; iridescence, dispersion, transmission roughness behavior, and several edge cases remain planned. |
...
| Additional texture slots | Partial | Remaining physical material extension maps, especially iridescence-related slots, are planned. |

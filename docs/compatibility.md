...
| `MeshPhongMaterial` | Partial | Blinn-Phong direct lighting with `specular`, `shininess`, and `specularMap` strength is supported. Scene-level environment/reflection probes feed an approximate specular reflection path, including `specularMap` masking. Material-level `envMap` still fails clearly; combine modes and exact transform/color-space edge cases remain planned. |
...
| Environment IBL | Supported | CPU precomputes diffuse irradiance, prefiltered specular, and BRDF LUT from equirectangular inputs and raw or encoded six-face cube reflection inputs. Standard/physical materials use diffuse/specular IBL, Phong materials use an approximate specular reflection path, and explicit `THREE.SRGBColorSpace`/`THREE.LinearSRGBColorSpace` is honored for scene-environment and reflection-probe LDR inputs; refraction and PMREM/CubeUV environment mappings fail clearly. |

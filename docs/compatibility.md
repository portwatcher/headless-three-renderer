...
| Shadow behavior parity | Partial | Supports common camera bounds, square map size, bias, normal bias, cast/receive flags, and alpha-tested casters. Non-square shadow map sizes, non-default shadow blur/radius settings, and `customDepthMaterial`/`customDistanceMaterial` shadow overrides fail clearly; native multiple-shadow-map rendering, rectangular maps, custom shadow material translation, and blur/radius support remain planned. |

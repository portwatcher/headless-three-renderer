...
| Shadow behavior parity | Partial | Supports common camera bounds, rectangular directional/spot/cascaded map sizes, square point-light cube-face maps, bias, normal bias, `shadow.radius` PCF scaling, PCF-irrelevant `shadow.blurSamples` inputs, cast/receive flags, and texture alpha-tested casters. Non-square point-light shadow map sizes and `customDepthMaterial`/`customDistanceMaterial` shadow overrides fail clearly; native multiple-shadow-map rendering, custom shadow material translation, and VSM-specific blur behavior remain planned. |

...
| `THREE.Sprite` / billboards | Partial | CPU-expanded into camera-facing quads with `SpriteMaterial` color, opacity, scene fog and `material.fog` opt-out, map with texture UV transforms and sRGB color-space decode, alphaMap with UV transforms, center, scale, rotation, size attenuation, layers, render ordering, and main-pass clipping. Explicit sprite shadow flags fail clearly; sprite shadow rendering and exact WebGL edge cases remain planned. |

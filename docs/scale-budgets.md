...
- Use `pnpm -C packages/renderer run profile:animated -- --all-modes --json` to capture same-settings reusable-renderer baselines for static, transform-only, material-only, mixed transform/material, and stable InstancedBufferGeometry workloads.

...
| Direct lights | 16 visible non-ambient lights | The supported native light-array budget is covered with 16 point lights plus ambient light. |
| Excess direct lights | 17 visible non-ambient lights | Scenes above the supported direct-light budget fail clearly instead of silently dropping lights. |
...
- The direct light limit is a real current renderer limit: more than 16 visible non-ambient lights fail clearly until native light arrays are expanded.
- Texture memory depends on decoded image dimensions, not just texture count. The CI texture-heavy scene uses tiny 4 x 4 textures to exercise many unique bindings without creating a large memory benchmark.

...
| Clipping planes | Medium | Render-option global planes and material-local `clippingPlanes`/`clipIntersection` affect the main pass for meshes, sprites, points, and lines, up to eight planes; `material.clipShadows` clips shadow-caster fragments in the shadow pass; and `options.localClippingEnabled: false` disables material-local planes while preserving global planes. Scene-level `THREE.ClippingGroup` inputs fail clearly. Remaining work is `ClippingGroup` semantics, alpha-to-coverage smoothing, and golden-tested parity. |

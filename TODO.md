...
| Clipping planes | Medium | Render-option global planes and material-local `clippingPlanes`/`clipIntersection` affect the main pass for meshes, sprites, points, and lines, up to eight planes; `clipShadows` and scene-level `THREE.ClippingGroup` inputs fail clearly. Remaining work is exact `WebGLRenderer.localClippingEnabled` state, `ClippingGroup` semantics, shadow-pass clipping support, alpha-to-coverage smoothing, and golden-tested parity. |

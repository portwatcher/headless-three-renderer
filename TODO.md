...
| More texture slots | High | Add remaining physical material extension maps, especially iridescence-related maps; deepen exact displacement edge-case parity. |
| More material classes | High | Deepen `ShadowMaterial` color-space/clipShadows parity, `PointsMaterial` cap/shadow parity, `SpriteMaterial` shadow parity, `MeshDistanceMaterial` point-shadow parity, `MeshToonMaterial` exact edge-case parity, `MeshPhongMaterial` environment/reflection parity, plus `LineDashedMaterial` edge cases. |
| Physical material parity gaps | High | Add/spec-check iridescence, dispersion, deeper attenuation parity, transmission roughness behavior, and clearcoat/normal/specular BRDF edge cases against Three.js. |
...
| Mip/filter/aniso settings | Medium | Base/background, normal/bump, metallic/roughness, emissive, AO/light, alpha, Phong specular, and packed physical-extension texture groups respect wrap modes plus `NearestFilter`/`LinearFilter`-family `magFilter`/`minFilter`; remaining work is generated mip chains, anisotropy, and conflicting per-channel sampler settings inside the same packed physical texture. |

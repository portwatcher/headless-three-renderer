...
- `THREE.ArrayCamera` sub-camera viewport composition for RGBA and target output.
...
| Task | Impact | Notes |
|---|---:|---|
| Array/cube camera support | Medium | `THREE.ArrayCamera` sub-cameras compose into RGBA and target output using each sub-camera viewport, including target depth readback; PNG ArrayCamera output and `THREE.CubeCamera` fail clearly. Remaining work is PNG composition, native environment capture, cube-camera targets, and exact WebGL viewport semantics. |

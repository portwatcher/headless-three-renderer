...
| Task | Impact | Notes |
|---|---:|---|
| Array/cube camera support | Medium | `THREE.ArrayCamera` sub-cameras compose into PNG, RGBA, and target output using each sub-camera viewport, including target depth readback; `THREE.CubeCamera` fails clearly. Remaining work is native environment capture, cube-camera targets, and exact WebGL viewport semantics. |

...
- Material-local `clippingPlanes`, `THREE.ClippingGroup`, plus render-option global `clippingPlanes`, up to eight world-space planes with over-budget inputs failing clearly, including local/group `clipIntersection`, group `clipShadows`, and `options.localClippingEnabled` opt-out.
...
- Directional, spot, point, and up to four directional cascaded shadow maps for a single shadow-casting light; common PCF shadow options including bias, normal bias, `radius`, and PCF-irrelevant `blurSamples` inputs are accepted; directional/spot/point sprite and point billboard shadow casting, mesh custom depth/distance shadow material alpha cutouts and displacement, and sprite/point billboard custom shadow material alpha cutouts are honored; additional shadow-casting lights and over-budget cascade hints fail clearly.
...
- `THREE.Sprite`/`SpriteMaterial` CPU billboard expansion with color, opacity, texture maps, scene fog, center, scale, rotation, size attenuation, layers, render ordering, and directional/spot/point shadow casting; `receiveShadow` fails clearly.
- Line material arrays with geometry groups, plus `LineDashedMaterial` dash/gap/scale patterns and custom `lineDistance` attributes for common line, line segment, line loop, and instanced line cases.
...
- `THREE.Fog` and `THREE.FogExp2` in mesh, shadow, sprite, point, and line material paths, using view-space depth fog with `material.fog` opt-out.
...
| Build a curated scene corpus | High | Initial generated invariant corpus covers transparent materials, skinning, morphs, avatar-style toon/skinned combinations, IBL, shadows, points/lines, LOD/groups, and pathological geometry in CI; glTF tests cover local geometry/material loading, encoded data-URI textures, and external buffer resolution in CI. Remaining work is committed glTF Sample Assets, external VRM/VRMA fixtures, and browser-generated references. |
...
| Render target depth output | Medium | Single-output targets with `depthTexture` populate RGBA8 normalized depth readback for plain depth targets, normalized scalar `Float32Array` data for `FloatType`, and scalar unsigned typed arrays for `UnsignedShortType`/`UnsignedIntType` depth textures, using depth-tested visible geometry; unsupported explicit depth texture types fail clearly; direct conformance covers near/far ordering, texture writeback, scissor clipping, alpha-map/alpha-test cutouts, and unsupported type validation. Remaining work is native hardware depth-texture readback and exact WebGL render-target depth semantics. |
...
| Viewport/scissor support | Medium | Render-option viewport/scissor rectangles and `target.viewport`/`target.scissor` with `target.scissorTest` are supported in output pixel coordinates; remaining work is exact WebGLRenderer state-machine parity and broader tiled-render ergonomics. |
...
| Provide asset loading helpers | Medium | `loadGltfFromFile`, `createNodeGltfLoader`, `createEncodedImageTextureLoader`, `EncodedImageTextureLoader`, `installLocalFileFetch`, and `resolveLocalAssetPath` help Node loaders parse local glTF/GLB files with plugin hooks and expose renderer-supported encoded texture buffers from local files, PNG/JPEG/WebP data URIs, and PNG/JPEG/WebP Blob URLs used by GLB bufferView images. Remaining work is broader helper coverage for KTX2/Basis pre-decode and deeper VRM convenience APIs. |
| Add end-to-end glTF/VRM examples | High | `examples/render-gltf.mjs` renders a local glTF/GLB asset fully in Node using the exported loader helpers, `examples/render-vrm.mjs` renders a local VRM avatar with optional VRMA animation through Pixiv loader plugins, tests render committed geometry/material and encoded-texture glTF fixtures through the exported local glTF helper, and the generated corpus includes a synthetic avatar-style toon/skinned scene. Remaining work is committed glTF Sample Assets plus external VRM/VRMA fixtures. |

...
- Material-local `clippingPlanes` plus render-option global `clippingPlanes` in the main pass, up to eight world-space planes, including local `clipIntersection` and `options.localClippingEnabled` opt-out.
...
- Scene background colors plus 2D and equirectangular texture backgrounds, including `backgroundIntensity`, approximate 2D texture `backgroundBlurriness`, equirectangular `backgroundRotation`, and equirectangular `environmentRotation`; non-equirectangular background rotations fail clearly.
...
| Scene background textures | Medium | 2D texture backgrounds are supported with output color-space conversion and approximate 2D texture blurriness; equirectangular background mappings sample from camera direction; cube/CubeUV background mappings fail clearly. Remaining work is rendering cube backgrounds, exact cube/equirect blur, newer mapping controls, and exact color-space parity. |
| Clipping planes | Medium | Render-option global planes and material-local `clippingPlanes`/`clipIntersection` affect the main pass for meshes, sprites, points, and lines, up to eight planes, and `options.localClippingEnabled: false` disables material-local planes while preserving global planes; `clipShadows` and scene-level `THREE.ClippingGroup` inputs fail clearly. Remaining work is `ClippingGroup` semantics, shadow-pass clipping support, alpha-to-coverage smoothing, and golden-tested parity. |
...
| Shadow behavior parity | High | Common `shadow.camera`, rectangular directional/spot/cascaded map sizes, square point-light cube-face maps, bias, normal bias, `shadow.radius` PCF scaling, cast/receive rules, and alpha-tested casters are supported; non-square point-light shadow map sizes, non-default shadow blurSamples settings, and custom shadow material overrides fail clearly. Remaining work is configurable blurSamples behavior, custom shadow material translation, transparency parity, and deeper golden-tested behavior. |

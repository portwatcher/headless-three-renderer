...
| Shadow behavior parity | High | Common `shadow.camera`, rectangular directional/spot/cascaded map sizes, square point-light cube-face maps, bias, normal bias, `shadow.radius` PCF scaling, PCF-irrelevant `shadow.blurSamples` inputs, cast/receive rules, and alpha-tested casters are supported; non-square point-light shadow map sizes and custom shadow material overrides fail clearly. Remaining work is VSM-specific blur behavior if a VSM shadow mode is added, custom shadow material translation, transparency parity, and deeper golden-tested behavior. |

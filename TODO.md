...
| Shadow behavior parity | High | Common `shadow.camera`, square map size, bias, normal bias, cast/receive rules, and alpha-tested casters are supported; non-square shadow map sizes and non-default shadow blur/radius settings fail clearly. Remaining work is rectangular map sizes, configurable radius/blur behavior, transparency parity, and deeper golden-tested behavior. |
...
| Large scene memory budget tests | Medium | Scale regression tests render many meshes, textures, and the supported 16-light budget in CI, and scenes with more than 16 visible non-ambient lights fail clearly. Remaining work is larger memory ceilings, texture-heavy stress cases, and platform-specific budget documentation. |

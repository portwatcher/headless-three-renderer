...
| More material classes | High | Material-level `envMap` reflection inputs fail clearly; scene-level environment/reflection probes are the supported IBL path, including direct conformance for `MeshPhongMaterial` specular environment reflection and `specularMap` masking. `Object3D.customDepthMaterial` and `customDistanceMaterial` are honored for shadow caster alpha-tested and displacement material inputs. `MeshToonMaterial` has direct conformance for fallback bands, gradient maps, base-map UV channels, emissive-map UV channels, light-map secondary UVs, and alpha-map cutouts. `ShadowMaterial` covers the current transparent receiver path with color, opacity, scene fog, fog opt-out, and output conversion. Remaining work is deepening `ShadowMaterial` exact parity, `PointsMaterial` cap/shadow rendering parity, `SpriteMaterial` shadow rendering parity, `MeshDistanceMaterial` point-shadow parity, broader custom shadow material translation, `MeshToonMaterial` browser-golden edge-case parity, `MeshPhongMaterial` material-level envMap/combine parity, plus `LineDashedMaterial` edge cases. |

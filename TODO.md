...
- `THREE.ArrayCamera` sub-camera viewport composition for PNG, RGBA, and target output, plus `THREE.CubeCamera` six-face cube target rendering.
...
| Task | Impact | Notes |
|---|---:|---|
| Array/cube camera support | Medium | `THREE.ArrayCamera` sub-cameras compose into PNG, RGBA, and target output using each sub-camera viewport, including target depth readback; `THREE.CubeCamera` renders six RGBA cube target faces into `WebGLCubeRenderTarget.texture.image`/`source.data`. Remaining work is cube-camera mip/depth targets, native environment-capture workflows, and exact WebGL viewport/face semantics. |

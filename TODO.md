...
| More material classes | High | Material-level `envMap` reflection inputs and `customDepthMaterial`/`customDistanceMaterial` shadow overrides fail clearly; scene-level environment/reflection probes are the supported IBL path. `ShadowMaterial` covers the current transparent receiver path with color, opacity, scene fog, fog opt-out, and output conversion. Remaining work is deepening `ShadowMaterial` exact parity, `PointsMaterial` cap/shadow rendering parity, `SpriteMaterial` shadow rendering parity, `MeshDistanceMaterial` point-shadow parity, custom shadow material translation, `MeshToonMaterial` exact edge-case parity, `MeshPhongMaterial` environment/reflection parity, plus `LineDashedMaterial` edge cases. |

...
| Render target depth output | Medium | Single-output targets with `depthTexture` populate RGBA8 normalized depth readback for plain depth targets, normalized scalar `Float32Array` data for `FloatType`, and scalar unsigned typed arrays for `UnsignedShortType`/`UnsignedIntType` depth textures, using depth-tested visible geometry; direct conformance covers near/far ordering, texture writeback, scissor clipping, and alpha-map/alpha-test cutouts. Remaining work is native hardware depth-texture readback and exact WebGL render-target depth semantics. |

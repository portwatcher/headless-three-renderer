...
- Base/background, normal/bump, metallic/roughness, emissive, AO/light, alpha, Phong specular, and packed physical-extension texture-group sampler wrap modes, `magFilter`/`minFilter` (`NearestFilter`/`LinearFilter` family), and compatible native texture anisotropy.
...
- Scene background colors plus 2D, equirectangular, and raw six-face cube texture backgrounds, including `backgroundIntensity`, approximate 2D texture `backgroundBlurriness`, equirectangular/cube `backgroundRotation`, and equirectangular `environmentRotation`; unsupported background rotations fail clearly.
...
| Scene background textures | Medium | 2D texture backgrounds are supported with output color-space conversion and approximate 2D texture blurriness; equirectangular and raw six-face cube `DataTexture` background mappings sample from camera direction; PMREM/CubeUV background mappings fail clearly. Remaining work is exact cube/equirect blur, encoded cube-face decode, newer mapping controls, and exact color-space parity. |

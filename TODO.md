...
- Base color, opacity, premultiplied alpha, base/sprite/point/line/matcap/emissive/light/toon gradient/physical color map, supported background texture, and LDR environment IBL sRGB/linear color-space handling, output color-space controls, base color `flipY`, normal/bump maps, metallic/roughness/specular maps, AO/light maps with selected UV channels, Phong specular maps with selected UV channels, toon gradient maps, alpha/displacement maps, alpha test, alpha hash, transparency sorting, side/culling, and texture wrap modes.
- Base color, supported background, sprite/point color and alpha, line, normal, bump, displacement, metallic/roughness, emissive, AO, light, specular, alpha, and physical-extension map texture UV transforms (`offset`, `repeat`, `rotation`, `center`, `flipY`, and explicit matrix), with direct explicit-matrix conformance for base/background, sprite/point color and alpha, line, and common mesh material maps.
...
| More material classes | High | Material-level `envMap` reflection inputs and `customDepthMaterial`/`customDistanceMaterial` shadow overrides fail clearly; scene-level environment/reflection probes are the supported IBL path. Remaining work is deepening `ShadowMaterial` exact parity, `PointsMaterial` cap/shadow rendering parity, `SpriteMaterial` shadow rendering parity, `MeshDistanceMaterial` point-shadow parity, custom shadow material translation, `MeshToonMaterial` exact edge-case parity, `MeshPhongMaterial` environment/reflection parity, plus `LineDashedMaterial` edge cases. |
| Physical material parity gaps | High | Iridescence and dispersion inputs fail clearly, and scene-color transmission uses a roughness/IOR-aware filter. Remaining work is adding/spec-checking unsupported features, deeper attenuation parity, exact transmission mip/bicubic parity, and clearcoat/normal/specular BRDF edge cases against Three.js. |
